...
public class CompositeAzureDevOpsService : IAzureDevOpsService
{
    ...
    public Task<int> GetExistingRelatedParentItemIdAsync(int workItemId, CancellationToken cancellationToken = default)
        => _relationService.GetExistingRelatedParentItemIdAsync(workItemId, cancellationToken);

    public Task<IEnumerable<WorkItem>> GetWorkItemsByQueryAsync(string wiqlQuery, CancellationToken cancellationToken = default)
    ...
    public Task<int> CreateReleaseTrainAsync(List<int> children, string title, int patternItemId = 0)
        => _workItemService.CreateReleaseTrainAsync(children, title, patternItemId);
